...
- [saas_week8_verification_checklist_zh.md](./saas_week8_verification_checklist_zh.md)（新增 delivery tracking panel、attention queue、返回 admin queue、organization drill-down、Week 8 readiness summary 以及 readiness drill-down 使用說明）
  ...
  - 當前驗收邊界仍以 `unit + contract + page + non-browser smoke` 為主，但已新增 138 條最小 true browser smoke，可透過 `npm run web:test:browser:smoke` 執行；Playwright 目前會先做一次 `next build`，再以本機 `next start` 啟動 3005 上的 production-backed server，避免長測試被 `next dev` watcher 中途重啟打斷。若只是在本地快速調單條 smoke，也可改用 `npm run web:test:browser:smoke:dev` 臨時切回 watcher-backed dev server，但需接受較高的中途重啟風險：
