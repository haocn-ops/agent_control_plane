...
export async function GET() {
  ...
  return proxyControlPlaneOrFallback(
    `/api/v1/saas/workspaces/${workspaceContext.workspace.workspace_id}/invitations`,
    {
      items: [],
      page_info: {
        next_cursor: null,
...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(
    `${baseUrl}/api/v1/saas/workspaces/${workspaceContext.workspace.workspace_id}/invitations`,
    {
      method: "POST",
      headers: {
