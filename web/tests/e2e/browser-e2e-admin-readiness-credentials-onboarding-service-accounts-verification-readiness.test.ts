...
test("browser readiness credentials onboarding->service-accounts->verification smoke keeps admin-readiness continuity explicit without overstating coverage", async () => {
  ...
  assert.match(browserSmokeSpec, /Step 2: Create service account/);
  ...
  assert.match(browserSmokeSpec, /Machine identities/);
  assert.match(browserSmokeSpec, /Credential sequence/);
  assert.match(browserSmokeSpec, /Step 5: Capture verification evidence/);
