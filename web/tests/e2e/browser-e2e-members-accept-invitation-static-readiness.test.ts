...
const browserSpecPath = path.resolve(webDir, "tests/browser/members-accept-invitation-static.smoke.spec.ts");

test("browser readiness members->accept-invitation static smoke keeps continuity explicit", async () => {
  ...
  assert.match(browserSmokeSpec, /Accept invite/);
