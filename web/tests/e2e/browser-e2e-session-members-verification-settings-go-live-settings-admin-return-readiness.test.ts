...
test(
  "browser readiness session->members->verification->settings->go-live->settings->admin smoke keeps return continuity explicit",
  async () => {
    const browserSmokeSpec = await readFile(browserSpecPath, "utf8");

    assert.match(
      browserSmokeSpec,
      /session -> members -> verification -> settings -> go-live -> settings -> admin keeps readiness return continuity/,
    );
