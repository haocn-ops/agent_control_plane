...
test(
  "browser readiness launchpad->session->members->verification->settings->go-live->admin smoke keeps return continuity explicit",
  async () => {
    const browserSmokeSpec = await readFile(browserSpecPath, "utf8");

    assert.match(
      browserSmokeSpec,
      /launchpad -> session -> members -> verification -> settings -> go-live -> admin keeps readiness return continuity/,
    );
