...
const browserSpecPath = path.resolve(
  webDir,
  "tests/browser/members-accept-invitation-verification-settings-verification-admin-return.smoke.spec.ts",
);

test(
  "browser readiness members->accept-invitation->verification->settings->verification->admin smoke keeps return continuity explicit",
  async () => {
    const browserSmokeSpec = await readFile(browserSpecPath, "utf8");

    assert.match(
      browserSmokeSpec,
      /members -> accept-invitation -> verification -> settings -> verification -> admin keeps readiness return continuity/,
    );
    assert.match(
      browserSmokeSpec,
      /\/members\?source=admin-readiness&week8_focus=credentials&attention_workspace=preview&attention_organization=org_demo&delivery_context=week8&recent_track_key=verification&recent_update_kind=verification&evidence_count=2&recent_owner_label=Ops/,
    );
