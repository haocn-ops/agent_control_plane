...
test("browser readiness onboarding->accept-invitation->usage smoke keeps continuity explicit", async () => {
  const browserSmokeSpec = await readFile(browserSpecPath, "utf8");

  assert.match(browserSmokeSpec, /onboarding -> accept-invitation -> usage keeps invite continuity/);
  assert.match(
    browserSmokeSpec,
