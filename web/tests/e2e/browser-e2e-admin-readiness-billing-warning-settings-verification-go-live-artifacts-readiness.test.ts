...
test(
  "browser readiness billing-warning settings->verification->go-live->artifacts smoke keeps continuity explicit without overstating coverage",
  async () => {
    ...
    assert.match(browserSmokeSpec, /Inspect artifacts evidence/);
    assert.match(browserSmokeSpec, /Generated output and evidence/);
