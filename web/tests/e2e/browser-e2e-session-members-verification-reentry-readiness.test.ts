...
test("session members verification re-entry browser batch stays wired into scripts and docs", async () => {
  ...
  assert.match(docsReadme, /web:test:browser:session-members-verification-reentry/);
  ...
  assert.match(executionPlan, /session-members-verification-reentry/);
