...
const browserSpecPath = path.resolve(
  webDir,
  "tests/browser/members-accept-invitation-verification-settings-verification-go-live-admin-return.smoke.spec.ts",
);

test(
  "browser readiness members->accept-invitation->verification->settings->verification->go-live->admin smoke keeps return continuity explicit",
  async () => {
