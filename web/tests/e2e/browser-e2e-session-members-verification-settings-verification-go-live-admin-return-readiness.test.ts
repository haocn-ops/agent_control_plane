...
test(
  "browser readiness session->members->verification->settings->verification->go-live->admin smoke keeps return continuity explicit",
  async () => {
    const browserSmokeSpec = await readFile(browserSpecPath, "utf8");

    assert.match(
      browserSmokeSpec,
      /session -> members -> verification -> settings -> verification -> go-live -> admin keeps readiness return continuity/,
    );
