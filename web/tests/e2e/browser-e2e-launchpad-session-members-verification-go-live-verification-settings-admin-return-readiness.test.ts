...
test(
  "browser readiness launchpad->session->members->verification->go-live->verification->settings->admin smoke keeps return continuity explicit",
  async () => {
    const browserSmokeSpec = await readFile(browserSpecPath, "utf8");

    assert.match(
      browserSmokeSpec,
      /launchpad -> session -> members -> verification -> go-live -> verification -> settings -> admin keeps readiness return continuity/,
    );
