...
const browserSpecPath = path.resolve(
  webDir,
  "tests/browser/onboarding-accept-invitation-verification-settings-verification-admin-return.smoke.spec.ts",
);

test(
  "browser readiness onboarding->accept-invitation->verification->settings->verification->admin smoke keeps return continuity explicit",
  async () => {
    const browserSmokeSpec = await readFile(browserSpecPath, "utf8");

    assert.match(
      browserSmokeSpec,
      /onboarding -> accept-invitation -> verification -> settings -> verification -> admin keeps readiness return continuity/,
    );
    assert.match(
      browserSmokeSpec,
