...
test("browser readiness members->accept-invitation->verification->go-live->admin smoke keeps continuity explicit", async () => {
  const browserSmokeSpec = await readFile(browserSpecPath, "utf8");

  assert.match(
    browserSmokeSpec,
    /members -> accept-invitation -> verification -> go-live -> admin keeps readiness return continuity/,
  );
  assert.match(
    browserSmokeSpec,
