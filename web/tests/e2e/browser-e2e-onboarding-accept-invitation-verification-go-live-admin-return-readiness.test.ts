...
test("browser readiness onboarding->accept-invitation->verification->go-live->admin smoke keeps continuity explicit", async () => {
  const browserSmokeSpec = await readFile(browserSpecPath, "utf8");

  assert.match(
    browserSmokeSpec,
    /onboarding -> accept-invitation -> verification -> go-live -> admin keeps invite continuity/,
  );
  assert.match(
    browserSmokeSpec,
