...
test("admin readiness demo-run branch -> verification -> go-live -> playground -> admin keeps readiness browser continuity", async ({
  page,
}) => {
  test.slow();
  ...
  const playgroundLink = page.getByRole("link", { name: "Revisit playground run" }).first();
