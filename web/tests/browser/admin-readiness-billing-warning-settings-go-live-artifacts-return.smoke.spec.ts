...
test("admin readiness billing warning branch -> settings -> go-live -> artifacts -> admin keeps readiness browser continuity", async ({
  page,
}) => {
  test.slow();
  ...
  const artifactsLink = page.getByRole("link", { name: "Inspect artifacts evidence" }).first();
  ...
  await expect(page).toHaveURL(/attention_workspace=preview/);
  await expect(page).toHaveURL(/attention_organization=org_preview/);
  await expect(page.getByRole("heading", { name: "Generated output and evidence" })).toBeVisible();
  await expect(page.getByText("Admin follow-up context")).toBeVisible();
  await expect(page.getByText("Focus Billing warning")).toBeVisible();
