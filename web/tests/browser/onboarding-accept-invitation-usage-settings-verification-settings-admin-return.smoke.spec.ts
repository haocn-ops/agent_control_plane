...
test(
  "onboarding -> accept-invitation -> usage -> settings -> verification -> settings -> admin keeps readiness return continuity",
  async ({ page }) => {
    test.slow();
    ...
    const verificationSettingsLink = page.getByRole("link", { name: "Review settings + billing" }).first();
    await expect(verificationSettingsLink).toBeVisible();

    await verificationSettingsLink.click();
