...
test("admin readiness credentials branch -> onboarding -> service-accounts -> verification -> admin keeps readiness browser continuity", async ({
  page,
}) => {
  test.slow();
  ...
  const serviceAccountsLink = page.getByRole("link", { name: "Step 2: Create service account" }).first();
  await expect(serviceAccountsLink).toBeVisible();
  await serviceAccountsLink.click();
  ...
  const verificationLink = page.getByRole("link", { name: "Step 5: Capture verification evidence" }).first();
