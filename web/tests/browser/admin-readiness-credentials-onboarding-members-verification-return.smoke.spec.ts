...
test("admin readiness credentials branch -> onboarding -> members -> verification -> admin keeps readiness browser continuity", async ({
  page,
}) => {
  test.slow();
  ...
  const membersLink = page.getByRole("link", { name: "Step 1: Invite first members" }).first();
