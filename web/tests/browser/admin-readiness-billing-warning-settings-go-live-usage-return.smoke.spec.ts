...
test("admin readiness billing warning branch -> settings -> go-live -> usage -> admin keeps readiness browser continuity", async ({
  page,
}) => {
  test.slow();
  ...
  const usageLink = page.getByRole("link", { name: "Confirm usage posture" }).first();
