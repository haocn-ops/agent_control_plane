...
test("admin readiness credentials branch -> onboarding -> playground -> verification -> admin keeps readiness browser continuity", async ({
  page,
}) => {
  test.slow();
  ...
  const playgroundLink = page.getByRole("link", { name: "Step 4: Run playground demo" }).first();
  ...
  await expect(page).toHaveURL(/attention_workspace=preview/);
  await expect(page).toHaveURL(/attention_organization=org_preview/);
  await expect(page.getByRole("heading", { name: "Prompt, invoke, inspect" })).toBeVisible();
  await expect(page.getByText("Admin follow-up context")).toBeVisible();
  await expect(page.getByText("Focus Credentials")).toBeVisible();
  await expect(page.getByText("Plan-limit checkpoint")).toBeVisible();
