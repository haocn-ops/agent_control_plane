...
test(
  "members -> accept-invitation -> onboarding -> usage -> settings -> verification -> settings -> admin keeps readiness return continuity",
  async ({ page }) => {
    test.slow();
    ...
    const reviewSettingsLink = page.getByRole("link", { name: "Review settings + billing" }).first();
    await expect(reviewSettingsLink).toBeVisible();
