...
test(
  ...
  async ({ page }) => {
    ...
    const settingsVerificationLink = page.getByRole("link", { name: "Capture verification evidence" }).first();
    await expect(settingsVerificationLink).toBeVisible();
    await settingsVerificationLink.click();
    ...
    await expect(page.getByRole("heading", { name: "Week 8 launch checklist" })).toBeVisible();
    await expect(page.getByText("Verification evidence lane")).toBeVisible();
    const reopenedSettingsLink = page.getByRole("link", { name: "Review settings + billing" }).first();
    await expect(reopenedSettingsLink).toBeVisible();
