...
test(
  ...
  async ({ page }) => {
    ...
    const returnSettingsLink = page.getByRole("link", { name: "Review billing + settings" }).first();
    await expect(returnSettingsLink).toBeVisible();
    await returnSettingsLink.click();
